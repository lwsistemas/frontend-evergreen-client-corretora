...
const EmprestimoIcone = (props) => (

    <RequestQuoteIcon sx={{ color: props.color }} />
            ...
            {items.map((item, index) => (
                <li
                    ...
                    style={{
                        ...
                        backgroundColor: item.selected ? '' : 'transparent',
                        borderRadius: item.selected ? '8px' : '0px',
                        borderLeft: item.selected ? "3px solid #FFC107" : "3px solid #FFC107"
                    }}
                >
                    <a href={item.link} className="d-flex" style={{ gap: '12px' }}>
                        <span
                            style={{
                                fontWeight: 'medium',
                                color: item.selected ? '#000' : '#C8C7C8',
                                paddingLeft: '10px'
...
const SidebarComponent = ({ selectedItem, onClose }) => {
    ...
    const [menuData, setMenuData] = useState([
        ...
        {
            name: t('Application_Emprestimos'),
            link: '/emprestimos',
            icon: (<EmprestimoIcone
                color={selectedItem === 'home' ? '#000' : '#C8C7C8'}
            />
            ),
            isOpen: false,
        ...
        ,
        {
            ...
            submenus: [
                { name: t("My data"), link: '/edituser' },
                { name: t('Order History'), link: '/user/history' },
                { name: t('My Wallet'), link: '/user/mywallet' },
                { name: t('My documents'), link: '/user/mydocuments' },
                { name: t('Bank data'), link: '/user/bank' },
            ],
            isOpen: false,
        },
        {
            name: t('Application_Deposito'),
            link: '#',
            icon: (<DepositFiatIcon
                color={selectedItem === 'home' ? '#000' : '#C8C7C8'}
            />
            ),
            submenus: [
                { name: t("Deposit fiat"), link: '/deposit' },
                { name: t('Deposit crypto'), link: '/account-deposit-cripto' },
            ],
            isOpen: false,
        },
        {
            name: t('Application_Saque'),
            link: '#',
            icon: (<WithoutFiatIcon
                color={selectedItem === 'home' ? '#000' : '#C8C7C8'}
            />
            ),
            submenus: [
        ...
        },
        {
            ...
            submenus: [
                { name: t("Application_TodosTickes"), link: '/support' },
                { name: t('Application_AbrirChamado'), link: '/ticket/add' },
            ],
            isOpen: false,
        }
    ]);
    ...
    const handleMenuClick = (menuIndex) => {
        const updatedMenuData = menuData.map((menu, index) => {
            if (index === menuIndex) {
        ...
        // Atualize o estado dos menus
        setMenuData(updatedMenuData);
    };


    return (
        <aside className="sidebar" style={{ zIndex: 9999, backgroundColor: "rgba(18, 19, 24, 0.96)", color: 'white', width: '200px', padding: '16px 0px', transition: 'height 1.5s', borderTopRightRadius: '10px', top: '62px', bottom: '145px' }}>
            {/* <div
                ...
                <div key={index}>
                    <li
                        style={{
                            ...
                            gap: '8px',
                            justifyContent: 'start',
                            backgroundColor: menu.isOpen ? '#FFC107' : 'transparent',
                            borderTopRightRadius: menu.isOpen ? '5px' : '0px',
                            borderBottomRightRadius: menu.isOpen ? '5px' : '0px',


                        }}

                    >
                        <a href={`${menu.link}`} className="d-flex" style={{ gap: '12px' }} onClick={() => handleMenuClick(index)}>

                            {menu.icon}
                            <span
                                style={{
                                    fontWeight: menu.isOpen ? 'bold' : "medium",
                                    color: menu.isOpen ? '#fff' : '#C8C7C8',
                                }}
                            >
                                {menu.name}
                            </span>
                    ...
                    <Submenu items={menu.submenus} isOpen={menu.isOpen} />
                </div>
            ))}
            <li
                style={{
                    ...
                    rubyAlign: 'center',
                    backgroundColor: selectedItem === 'close' ? '#FFC107' : 'transparent',
                    borderRadius: selectedItem === 'close' ? '8px' : '0px',
                }}
            >
                <a href="#close" className="d-flex" style={{ gap: '12px' }} onClick={onClose}>
                    <span
                        style={{
                            fontWeight: 'medium',
                            color: selectedItem === 'close' ? '#000' : '#C8C7C8',
                        }}
                    >
                        <MenuOpenIcon />
                    </span>
