...
export default function Confirm2FA(props) {
  const {user}=props
  ...
  const handleSubmit2FA = async () => {
    awaitActions(true)
    try {
      const result = (await axios.post('/confirmRemove2FA', { authKey:user.authKey, token }))
      props.setShowQrcode('block')
      props.setShowCancel('none')
      setShowSucess('block')
      setShowComfirm('none')
    } catch (error) {
            ...
            <div class="confirm-body">
              <div class="auth-form">
                ...
                <Alert style ={{color:"rgb(16 216 118)",backgroundColor:"transparent"}}icon={<CheckIcon fontSize="inherit" />}severity="success">
                    {t("Authenticator successfully removed.")}
                  </Alert>
                  </div >
              <div class="card-body">

                <div class="text-center">
                  <button disabled={disabledButton} onClick={handleClose} className="btn btn-success btn-block">Confirm </button>
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="confirm-body">
              <div class="auth-form">
                ...
                <div class="card-body">
                  ...
                  <p class="titleEmail">{t("Enter the code sent to the email")} <strong>{user.email}</strong></p>
